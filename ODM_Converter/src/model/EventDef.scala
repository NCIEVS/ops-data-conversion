...
class EventDef(val name: String, val repeating: String, val eventType: String) extends Sourced with OIDEntity with References {
  
  def this(oid: String, name : String, repeating: String, eventType: String) = {
    this(name, repeating, eventType)
    this.oid = oid
  }
  
  override def toString(): String = "EventDef[" + oid + ", name=" + name + ", src=" + source + "]"
}
