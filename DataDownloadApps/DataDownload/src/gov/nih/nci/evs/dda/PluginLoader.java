...
/**
 * Class taken from Java Code Ranch
...
public class PluginLoader extends ClassLoader{
    ...
    public Class loadClass (String name, String packageName) throws ClassNotFoundException {
        return loadClass(name, packageName, true);
    }
    ...
    public Class loadClass (String classname, String packageName, boolean resolve) throws ClassNotFoundException {
        try {
            //          String packagedClass = packageName + "." + classname;
            // Our ClassLoader superclass has a built-in cache of classes it has
            // already loaded. So, first check the cache.
            Class c = findLoadedClass(classname);
            if(c==null){
                c= findLoadedClass(packageName+"." + classname);
            }
            ...
            try{
            if (c == null) {
                try { c = findSystemClass(packageName + "." +classname); }
                catch (Exception ex) {}
            }}
            catch (Exception ex){
            ...
            if(c==null){
                c= findClass(packageName + "." + classname);
            }
            ...
            if (c == null) {
                ...
                // Now call an inherited method to convert those bytes into a Class
                c = defineClass(packageName+"." +classname, classbytes, 0, length);
            }
